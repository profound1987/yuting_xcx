...
6. 小程序把完整设备号发送给设备。
7. 设备使用本地烧录信息或设备安全区校验该设备号是否属于自己。
...
12. 小程序通过 BLE 把 `ssid`、`password`、`deviceNo` 和必要的配网会话信息发送给设备。
13. 设备尝试连接 Wi-Fi。
...
只有设备已经连接 Wi-Fi、成功连接云端服务器、云端确认设备在线，并且 `device.bind` 返回成功，手机端才认为配置成功。不能因为 BLE 已经下发 Wi-Fi 就提前把设备加入设备列表。
...
用户配置设备时，云端 `device.prepareConfigure` 至少要求 `status = registered`，并根据 `bindStatus` 和 `ownerUserId` 判断是否允许进入 BLE 配网。最终 `device.bind` 至少要求设备已完成配网、已连接云端、云端认证通过、`bindStatus = unbound`。如果当前手机号对应的用户还不存在，云端应先创建用户，再执行最终绑定；只有用户创建成功、设备归属写入成功、默认配置和绑定审计记录创建成功后，`device.bind` 才返回成功。
...
- 配置页调用 `device.prepareConfigure` 做云端归属检查；设备已绑定到他人时停止流程并提示联系管理员。
- 未绑定设备进入 BLE 配网流程，只展示蓝牙名称以 `ytsh-` 开头的设备。
- 小程序通过 BLE 先发送设备号用于设备端校验，再发送 Wi‑Fi SSID 和密码。
