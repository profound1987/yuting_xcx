...
类型码在整个云汀设备平台内全局唯一，不是只在单个小程序内唯一：

| 产品族 | 已有/规划类型码 | 接入方式 | 状态 |
| --- | --- | --- | --- |
| 云汀智家 `smart_home` | `AW / ES / LC / SP / GW` | `ble_wifi`，BLE 配网后通过 MQTT/HTTPS 通信 | 已实现 |
| 云汀智车 `smart_car` | `SL` = Smart LED | `ble_only`，手机通过 BLE 与设备通信 | 已实现 |
| 云汀智车 `smart_car` | `SS` = Smart Speaker | 由具体硬件定义，预计可复用通用 BLE 绑定 | 规划保留，尚未实现 |
| 云汀智人等后续产品 | 待登记 | 按设备能力选择接入方式 | 规划 |

因此 Smart LED 固定使用 `SL`，`SS` 保留给 Smart Speaker，不能把两种产品共用同一个类型码。所有产品族共用服务器的用户、生产台账、绑定关系和管理能力，通过 `product_family` 与 `onboarding_mode` 选择各自接入流程。
...
生成的二维码 PNG 图片应在二维码下方显示两行人工可读文字，方便贴纸、包装盒或售后手工核对：

```text
设备号:YT-AW-00100-33D7
PIN码:123456
```
