...
设备编号规范见 [device-numbering-design.md](device-numbering-design.md)。小程序与 HTTPS 服务端总体设计见 [system-design.md](system-design.md)。MQTTS 首版落地、Broker 选型、Topic、兼容 HTTPS `command.pull` 和自测流程见 [mqtts-rollout-plan.md](mqtts-rollout-plan.md)。
...
- 首版 Broker 域名固定使用 `yutingsmarthome.xin:8883`；`mqtt.yutingsmarthome.xin` 尚未配置 DNS，暂不用于设备端。
- 证书过期前要有 OTA 或双证书轮换方案。

当前 MQTTS 联调不允许把“跳过证书校验”作为正式方案。设备端应使用 `MBEDTLS_SSL_VERIFY_REQUIRED`，CA 使用 Let’s Encrypt `ISRG Root X1`。
...
1. 小程序调用 `device.prepareConfigure`，云端只检查设备号、生产台账和绑定归属，并创建 `provisionSessionId`；云端不接收、不保存、不校验设备 PIN。
2. 设备进入配网模式后通过 BLE 广播，蓝牙名称必须以 `ytsh-` 开头。
3. 小程序连接 BLE 设备后，不直接发送 PIN，而是用 `deviceNo + PIN + fixedBleSalt` 派生 `YTS-BLE/1` 下行 AES-CCM key。
4. 设备使用本地烧录信息校验该 `deviceNo` 是否属于自己，并通过成功解密首个 AES-CCM 加密帧确认对方知道正确 PIN；校验失败时返回明文 Notify：`DEVICE_NO_MISMATCH`、`DEVICE_PIN_INVALID` 或 `DEVICE_VERIFY_FAILED`。
5. 小程序确认手机当前 Wi‑Fi，并在 `YTS-BLE/1` 加密帧内发送 `ssid`、`password`、`deviceNo` 和配网会话信息；PIN 不再作为 BLE 明文字段发送。
...
截至 2026-06-09，当前小程序和服务端采用“配网会话 + 设备认证上线 + 小程序轮询 + 最终绑定强校验”的结构，并引入 PIN 与 `not_provisioned/未入网` 状态。方案 B 中，PIN 只存在于小程序本地和设备端，用于 `YTS-BLE-PIN/1` 派生 BLE AES key；服务端不接收、不保存、不校验 PIN。

- 小程序通过 BLE 向设备写入配网和本地控制消息；设备通过明文 JSON Notify 返回能力、Wi‑Fi 连接状态或本地控制 ACK。Notify 不携带 Wi‑Fi 密码、PIN 或其他敏感字段，因此本期不做上行加密，也不需要上行 nonce。
  - `ble.hello`：能力确认帧，不包含 PIN。
  - `provision.wifi`：在 `YTS-BLE/1` AES-128-CCM 加密 BLE 下行帧中发送 `deviceNo`、`provisionSessionId`、`apiUrl`、`secureProtocol`、`heartbeatIntervalMs`、`ssid`、`password` 和时间戳。
  - `wifi.status` / `provision.wifi.status`：设备通过明文 JSON Notify 返回 Wi‑Fi 扫描、连接成功或失败状态，供小程序决定是否继续轮询云端。
  - `local.command`：未入网或离线时的本地控制命令，使用 `YTS-BLE/1` AES-128-CCM 加密下行帧。
  - `local.command.ack`：设备通过明文 JSON Notify 返回本地控制最终执行结果；所有本地控制命令都必须等执行完成后再 ACK，`status` 使用终态 `succeeded` 或 `failed`。
- 本期不兼容旧明文 BLE 命令 `verifyDeviceNo` / `provisionWifi` / `control.command`。如需设备端临时调试，可以在固件内做开发开关，但正式联调和出厂固件必须关闭。
- PIN 是 BLE 配网和本地授权的强制近场持有证明。设备号和 BLE 广播名称都不是秘密；没有 PIN 时，邻近人员可能仅凭扫描到的广播名称尝试配网。
- PIN 必须保存在设备安全存储器中；生产台账登记 `deviceNo`、PIN 和设备云端通信 AES 密钥 `deviceKey`。二维码或铭牌可以展示 `deviceNo + pin`，但不能包含 `deviceKey`。
- 服务端不需要 PIN 字段；`device.prepareConfigure` / `device.bind` / `device.addUnprovisioned` 都不要求 `pin`。
- 小程序当前 `waitCloudOnline()` 会轮询 `device.checkProvisionStatus`，只有返回 `DEVICE_READY_TO_BIND` 后才调用 `device.bind`。
- 如果已经扫描/连接到 BLE 设备但 Wi‑Fi 或云端认证失败，小程序允许用户先调用 `device.addUnprovisioned` 加入“我的设备”；此时设备状态为 `not_provisioned/未入网`，不是在线或离线。
...
当前 BLE GATT 传输仍使用以下 Service / Characteristic，但其承载内容从“明文 JSON”升级为 `YTS-BLE/1` 帧。BLE 不承载 MQTT，不使用 MQTT Topic、QoS、retain 或 `command.pull`；MQTT/MQTTS 只属于设备联网后的云端长连接通道。

BLE Service / Characteristic：

| 名称 | UUID | 方向 | 说明 |
| --- | --- | --- | --- |
| Provision Service | `0000FFF0-0000-1000-8000-00805F9B34FB` | - | BLE 本地安全通道服务 |
| Write Characteristic | `0000FFF1-0000-1000-8000-00805F9B34FB` | 小程序 -> 设备 | 小程序写入 `YTS-BLE/1` 握手帧和加密请求帧 |
| Notify Characteristic | `0000FFF2-0000-1000-8000-00805F9B34FB` | 设备 -> 小程序 | 设备返回明文 JSON 行：`ble.ready`、`provision.wifi.status`、`local.command.ack` 等状态与 ACK |

BLE 帧采用 UTF-8 JSON + `\n`，小程序写入时按 20 字节或 MTU 协商结果分片；设备 Notify 也按 JSON 行返回，便于小程序拼包解析。JSON 只是 BLE 帧编码和首版业务 payload 编码，不代表 BLE 要模拟 MQTT。正式设备必须把小程序下行的 `provision.wifi`、`local.command` 等敏感内容放入 `YTS-BLE/1` AES-128-CCM 加密帧中；设备上行 Notify 本期保持明文 JSON 状态，不携带敏感内容，不需要 nonce/ciphertext/tag。

#### 5.2.1.1 `YTS-BLE/1` 分层边界

| 层次 | 云端控制通道 | BLE 本地通道 |
| --- | --- | --- |
| 传输 | HTTPS `device.secureMessage` 或 MQTTS | BLE GATT Write/Notify |
| 路由 | HTTP API type 或 MQTT Topic | 已连接的近场 GATT 连接，不存在 Topic |
| 安全外壳 | `YTS-SEC/1`，使用设备 `deviceKey` 的 AES-128-CCM | 小程序下行使用 `YTS-BLE/1` + `YTS-BLE-PIN/1` 派生的 AES-128-CCM；设备上行 Notify 本期为明文状态 JSON |
| 应用语义 | `commandType`、`params`、`ttlSeconds`、`command.ack` | 复用同一套 `commandType`、`params`、`ttlSeconds` 和 ACK 状态语义 |
| 适用场景 | 设备已联网，服务端可审计和排队 | 配网、未入网、离线兜底、近场调试 |

因此，BLE 本地控制不走 MQTT，也不复用 MQTT `command.pull` 流程；但设备端执行器层应复用云端命令的业务解析代码。例如 `watering.manual.start` 在云端和 BLE 中含义完全一致，只是外层传输与安全会话不同。

#### 5.2.1.2 PIN 的正式用法：`YTS-BLE-PIN/1`

为降低设备端实现复杂度，BLE PIN 加密固定为 `YTS-BLE-PIN/1`。生产台账只需要登记：

- `deviceNo`
- 设备 PIN
- 设备云端通信 AES 密钥 `deviceKey`

BLE 不需要生产登记独立 `bleSalt`。`bleSalt` 是协议常量，固定写在小程序和设备固件中，只要双方使用同一个值即可。

安全边界必须明确：`YTS-BLE-PIN/1` 实现简单，但如果攻击者抓到有效 BLE 密文，可以离线枚举 PIN。因此生产设备必须使用随机 8 位 PIN，设备端必须做 PIN 失败限速；本期暂不实现 `bleControlTicket`，所有 BLE 配网和本地控制都只走 PIN 模式。
...
| --- | --- |
| 协议名 | `YTS-BLE-PIN/1` |
| 套件名 | `YTS-BLE-PIN-SHA256-AES128CCM-V1` |
| Hash | SHA-256 |
| 固定 BLE salt | `YUNTING-ZHIJIA-BLE-PIN-KEY-V1` |
| Key 派生 | `bleAesKey = first16Bytes(SHA256(deviceNo + "|" + pin + "|" + fixedBleSalt))` |
| 会话 AEAD | 小程序下行使用 AES-128-CCM，nonce 12 字节，tag 16 字节；设备上行 Notify 本期明文 JSON |
| AES 要求 | 协议层必须是 AES-CCM；如 MCU 只有 AES-ECB 单块硬件能力，可用它实现 CCM，但不得把协议降级为裸 AES-ECB |
| PIN 预处理 | 4-8 位 ASCII 数字；生产建议随机 8 位；禁止空格、全角数字和非数字字符 |
| JSON 编码 | 字段字典序、无多余空格、UTF-8 编码；测试脚本以 Python `json.dumps(sort_keys=True, separators=(",", ":"))` 为准 |

派生流程固定如下：

```text
fixedBleSalt = "YUNTING-ZHIJIA-BLE-PIN-KEY-V1"
keyMaterial = deviceNo + "|" + pin + "|" + fixedBleSalt
keyHash = SHA256(keyMaterial)
bleAesKey = keyHash[0:16]
```
...
BLE 握手可压缩为能力确认，不再需要 client/device proof：
...
`ble.ready` 是明文 Notify，只表示设备支持该 BLE 协议，不表示 PIN 已验证。真正的 PIN 验证发生在设备用 `bleAesKey` 成功解密并校验第一帧 AES-CCM tag 时；解密失败即视为 PIN 错误。

会话建立后，所有小程序下行敏感消息使用统一加密帧。设备上行 Notify 本期不使用该外层字段，仍是明文 JSON 行。
...
```json
{
  ...
  "ts": 1710000000000,
  "nonce": "hex(12B)",
  "ciphertext": "base64url(AES-CCM ciphertext)",
  "tag": "base64url(16B)"
}
```

AAD 必须使用 canonical JSON，字段固定为 `deviceNo`、`msgType`、`nonce`、`proto`、`seq`、`ts`，按字段名字典序输出，UTF-8 编码，不添加空格。AAD 只用于小程序下行 AES-CCM 加密帧。小程序下行 nonce 为 12 字节，设备端不需要解析其内部结构，只需要校验长度，并在同一 `bleAesKey` 下拒绝重复 `seq/nonce`。设备上行明文 Notify 不需要 nonce。

设备端必须对 PIN 失败做限速：连续 5 次 AES-CCM tag 校验失败后锁定 BLE 配网/控制 5 分钟；锁定期间仍可广播，但必须拒绝写入或返回统一错误 `DEVICE_PIN_LOCKED`。

##### `YTS-BLE-PIN/1` 测试向量

完整可复现生成脚本：`tools/yts_ble_pin_vector.py`。
...
aeadNonce = 000102030405060708090a0b
```
...
```text
AAD = {"deviceNo":"YT-AW-00000-A324","msgType":"provision.wifi","nonce":"000102030405060708090a0b","proto":"YTS-BLE/1","seq":1,"ts":1710000000000}
plaintext = {"apiUrl":"https://yutingsmarthome.xin/api","deviceNo":"YT-AW-00000-A324","heartbeatIntervalMs":90000,"password":"wifi-password","provisionSessionId":"ps_test_vector_001","secureProtocol":"YTS-SEC/1-AES-128-CCM","ssid":"Home-WiFi","ts":1710000000000,"type":"provision.wifi"}
```

AES-128-CCM 输出：

```text
ciphertext = 
tag = b37950196d797f2ad41f4586825d668e
```

设备端实现验收标准：同一组输入必须得到完全相同的 `bleAesKey`、`ciphertext` 和 `tag`。

#### 5.2.1.3 加密后的配网 payload

`provision.wifi` 解密后的 JSON payload：
...
#### 5.2.1.4 加密后的 BLE 本地控制 payload

BLE 本地控制只用于未入网、离线兜底或近场调试。设备在线时应优先走云端 HTTPS/MQTTS 控制，以便服务端排队、审计和状态闭环。

`local.command` 解密后的 JSON payload 复用云端命令语义：
...
设备处理后通过明文 JSON Notify 返回 `local.command.ack`。本地控制 ACK 必须等命令执行完成后才返回；对于 `watering.manual.start`，设备需要等 `durationSeconds` 对应的浇水动作结束后返回 `status=succeeded`，如果中断、超时或执行失败则返回 `status=failed` 和错误 `code/message`。本期小程序收到终态 ACK 后才显示执行成功或失败，不再通过下一次查询确认本地 BLE 控制结果。
...
```json
{
  ...
  "result": {
    ...
    "completedAt": 1710000011000
  }
}
```

设备后续恢复联网时，应通过普通 `telemetry.report` 上报最新状态；如需审计本地操作，可额外上报 `local.command.audit`。BLE 本地控制不创建 MQTT Topic，不通过 `command.pull` 拉取，也不直接生成云端 `device_commands` 记录，除非小程序或设备后续把本地执行结果同步给服务端。

#### 5.2.1.5 BLE 控制授权策略

- 本期暂不实现 `bleControlTicket`，所有 BLE 配网和本地控制都只使用 `YTS-BLE-PIN/1`。
- `not_provisioned/未入网` 设备：必须用设备号、PIN 和固定 BLE salt 派生 `bleAesKey`，成功校验 AES-CCM tag 后才允许控制。
- 已绑定但离线设备：本期同样只走 PIN 模式；小程序若本地没有该设备 PIN，需要用户重新输入设备标签上的 PIN。
- BLE 本地控制能力限制在安全风险较低的命令集合内，例如手动浇水开始/停止；不允许恢复出厂、重置密钥或修改高风险参数。
- 后续如要提高离线控制授权强度，再单独扩展云端签发的短期 `bleControlTicket`，本期设备端无需实现。

小程序控制弹窗仍显示三步：扫描蓝牙设备、将数据发送给设备、最终结果。

设备 Wi‑Fi 连接状态明文 Notify 成功示例：
...
1. 小程序完成 `YTS-BLE/1` 握手后，写入加密 `provision.wifi` 帧。
2. 写入后先等待设备通过明文 JSON Notify 返回 `wifi.status` 或 `provision.wifi.status`。
3. 如果 `status=failed`，立即停止配网流程并提示 `code/message` 对应错误原因。
4. 如果 `status=connected`，把 Wi‑Fi 步骤标记为成功，然后开始轮询 `device.checkProvisionStatus`。
5. 如果超时未收到 Wi‑Fi 状态，提示“未收到设备 Wi‑Fi 连接结果”，用户可重新配网。
6. 即使 Wi‑Fi 已连接，小程序仍不能直接绑定；最终绑定仍以云端收到 AES-CCM `provision.result` 为准。
...
| `heartbeatIntervalMs` | number | 建议 | 云端要求设备入网后的心跳周期，单位 ms；应与 BLE 加密 `provision.wifi` 中下发值一致 |
...
小程序通过 `YTS-BLE/1` 加密 `provision.wifi` 帧把 `provisionSessionId`、`apiUrl`、`secureProtocol`、`heartbeatIntervalMs` 和 Wi‑Fi 信息一并写给设备。`heartbeatIntervalMs` 是云端要求该设备入网后使用的心跳周期，设备后续 `telemetry.report` 应按此周期上报；云端也会按该周期计算离线超时。配网成功后，小程序不要直接绑定，而是轮询：
...
1. 设备首次配网成功后，通过 HTTPS `device.secureMessage` 上报 `provision.result`。
2. 云端完成 AES-128-CCM 认证解密后，在加密 `provision.ack` payload 中返回 `mqtt` 对象。
3. 设备断电重启或本地 MQTT 凭据丢失时，通过 HTTPS `device.secureMessage` 上报 `bootstrap.request`。
4. 云端完成 AES-128-CCM 认证解密后，在加密 `bootstrap.ack` payload 中返回 `mqtt` 对象。
5. 设备使用 `mqtt` 对象连接 MQTTS Broker。
6. Broker 通过 ACL 限制该凭据只能访问本设备 Topic。

首版使用固定 Broker 密码 MVP 机制：

- `mqtt.username = deviceNo`。
- `mqtt.password` 由服务器 `.env` 中的 `YT_MQTT_DEVICE_PASSWORD` 提供，只通过 `YTS-SEC/1` 加密响应下发给真实设备。
- `mqtt.password` 不能等于 eFuse AES `deviceKey`。
- `mqtt.password` 不能写入小程序、二维码、公开文档、日志或聊天记录。
- Broker ACL 根据设备号限制 Topic 读写范围。
- 后续量产应迁移到短期 token 或 mTLS。

`bootstrap.ack` 或 `provision.ack` 解密后的 `mqtt` 对象示例：
...
TLS 要求：设备端正式 MQTTS 必须启用证书校验，即 mbedTLS 使用 `MBEDTLS_SSL_VERIFY_REQUIRED`。首版 CA 为 Let’s Encrypt `ISRG Root X1`，服务器会在 `mqtt.tls.caPem` 中返回完整 PEM；设备端也可以把该根 CA 固化在固件或安全分区中。联调阶段不推荐跳过证书校验；如临时跳过，只能作为本地调试开关，不得进入正式固件。
...
| --- | --- | --- | --- | --- | --- |
| 通用 | 配网成功/失败 | 上行 | `provision.result` | HTTPS `device.secureMessage` | 是 |
| 通用 | 配网确认 | 下行 | `provision.ack` | HTTPS 响应 | 否 |
| 通用 | 重启/凭据恢复请求 | 上行 | `bootstrap.request` | HTTPS `device.secureMessage` | 否 |
| 通用 | 重启/凭据恢复确认 | 下行 | `bootstrap.ack` | HTTPS 响应 | 否 |
...
心跳周期由云端下发并以 `heartbeatIntervalMs=90000` 为首版统一值。小程序 BLE 加密 `provision.wifi`、HTTPS `provision.ack` 和 HTTPS `bootstrap.ack` 都应保持同一值。设备端优先使用最近一次云端加密响应中的 `heartbeatIntervalMs`；如果没有拿到云端值，默认使用 90000ms。云端按连续 2 个心跳周期未见设备消息判定离线，因此首版离线超时约 180 秒。传感器变化明显、设备业务状态变化、故障发生或电池电量低时应立即额外上报。心跳报文的目标不是上传大量历史数据，而是让云端判断设备是否在线、网络是否稳定、电源是否正常、传感器和执行器是否健康。
