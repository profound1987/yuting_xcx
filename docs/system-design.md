...
功能职责：按设备真实能力管理自动浇水策略。

业务逻辑：

- 用户必须拥有该设备。
- 设备必须已上报 `capabilities`，小程序和服务端都按 `features` 动态决定可配置项。
- 自动策略使用新配置结构：`enabledFeatures`、`automationMode`、`features`。
- 首次绑定后不自动生成真实默认配置；推荐值只能作为 placeholder 或用户主动填入草稿。
- 保存配置成功只表示云端已接受并生成 `watering.config.set` 命令；设备 ACK `succeeded` 后，配置才进入 `synced`。

### 6.7 手动浇水控制模块

功能职责：用户点击开始后生成一次设备命令，由设备确认后执行。

业务逻辑：

- 用户必须拥有该设备。
- 设备必须在线，离线时返回不可操作。
- 云端生成 `watering.manual.start` 指令，接口成功返回 `COMMAND_ACCEPTED` 和 `commandId`。
- 小程序必须通过 `device.getCommandStatus` 等待命令进入 `succeeded`，不能把 API 成功当作水泵已经启动。
- 设备确认后更新指令状态和设备最近浇水时间。
- 用户点击停止时生成 `watering.manual.stop` 指令，设备 ACK 成功后才展示已停止。
- 如果设备未确认，客户端应展示“等待设备响应”，不能假定设备已经执行。
...
这些值可以先作为 MVP 参数范围或推荐值，后续根据真实设备和用户使用情况调整；推荐值不能自动写成设备真实配置。